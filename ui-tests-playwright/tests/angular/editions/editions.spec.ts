...
test.describe('EDITIONS', () => {
    ...
    test.describe('CRUD', () => {
        ...
        test('should display modal on click to "New" button', async () => {
            ...
            await editionsPage.waitForModal('createModal');
        ...
        test('should show error when form is saved before required inputs are filled', async () => {
            await editionsPage.clickWithQuerySelector('#EditEdition_IsPaid');
        ...
        test('should hide error when form is properly filled', async () => {
            ...
            await editionsPage.triggerValidation('#EditionDisplayName', '#DailyPrice', '#WeeklyPrice', '#MonthlyPrice', '#AnnualPrice');
        ...
        test('should display modal on click to "Edit" button', async () => {
            ...
            await editionsPage.waitForResponse();
            await editionsPage.waitForModal('editModal');
        ...
        test('should save changes to record when "Save" button is clicked', async () => {
            await editionsPage.fillInputs({ '#EditionDisplayName': 'changed_name' });
            await editionsPage.saveForm();
            await editionsPage.waitForResponse();
        ...
        async function triggerValidation() {
            await fillInputsWithValidValues();
            await editionsPage.clearInputs('#EditionDisplayName', '#DailyPrice', '#WeeklyPrice', '#MonthlyPrice', '#AnnualPrice');
        }

        function fillInputsWithValidValues() {
            return editionsPage.fillInputs({
                '#EditionDisplayName': 'test',
                '#DailyPrice': '1',
                '#WeeklyPrice': '2',
                '#MonthlyPrice': '3',
                '#AnnualPrice': '4',

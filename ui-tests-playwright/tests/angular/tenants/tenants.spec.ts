...
test.describe('TENANTS', () => {
    ...
    test.describe('CRUD', () => {
        ...
        test('should display modal on click to "New" button', async () => {
            ...
            await tenantsPage.waitForModal('createModal');
        ...
        test('should hide error when form is properly filled', async () => {
            ...
            await tenantsPage.wait(1000);
            await tenantsPage.triggerValidation('#TenancyName', '#Name', '#AdminEmailAddress', '#adminPassword', '#AdminPasswordRepeat');
        ...
        test('should display modal on click to "Edit" button', async () => {
            ...
            await tenantsPage.waitForResponse();
            await tenantsPage.waitForModal('editModal');
            await tenantsPage.replaceLastColoumnOfTable();
        ...
        async function triggerValidation() {
            await fillInputsWithValidValues();
            await tenantsPage.clearInputs('#TenancyName', '#Name', '#AdminEmailAddress', '#adminPassword', '#AdminPasswordRepeat');
        }

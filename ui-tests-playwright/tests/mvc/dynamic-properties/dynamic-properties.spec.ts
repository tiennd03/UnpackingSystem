...
test.describe('DYNAMICPROPERTIES', () => {
    ...
    test.describe('CRUD', () => {
        ...
        test('should render the initial list', async () => {
            ...
            await expect(page).toHaveScreenshot(DYNAMICPROPERTIES_CRUD_LIST);
        });

        /* Step 2 */
        test('should display modal on click to "New" button', async () => {
            await dynamicPropertiesPage.clickByTextExact('Add New Dynamic Property');
            await dynamicPropertiesPage.waitForModalContent();

            await expect(page).toHaveScreenshot(DYNAMICPROPERTIES_CRUD_NEW_MODAL);
        });

        /* Step 3 */
        test('should save record when "Save" button is clicked', async () => {
            await dynamicPropertiesPage.fillInputs({ 'input[name=propertyName]': 'test' });
            await dynamicPropertiesPage.fillInputs({ 'input[name=displayName]': 'testDisplay' });
            await dynamicPropertiesPage.selectOptionByValue('.modal.show select', 'SINGLE_LINE_STRING');
        ...
        test('should display modal on click to "Edit" button', async () => {
            await dynamicPropertiesPage.triggerDropdownAction('Edit');
            await dynamicPropertiesPage.waitForResponse('CreateOrEditModal');
            await dynamicPropertiesPage.waitForModalContent();
        ...
        test('should save changes to record when "Save" button is clicked', async () => {
            await dynamicPropertiesPage.fillInputs({ 'input[name=propertyName]': 'changed_name', 'input[name=displayName]': 'changed_display_name' });
            await dynamicPropertiesPage.saveForm();
            await dynamicPropertiesPage.waitForResponse();
        ...
        test('should display warning on click to "Delete" button', async () => {
            await dynamicPropertiesPage.openActionsDropdown(1);
            await dynamicPropertiesPage.waitForDropdownMenu();
            await dynamicPropertiesPage.triggerDropdownAction('Delete');

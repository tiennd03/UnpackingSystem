...
using prod.Migrations.Seed.Host;
...
namespace prod.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : prodAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<prodDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{prodConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{prodConsts.ConnectionStringName}", constring);
        }

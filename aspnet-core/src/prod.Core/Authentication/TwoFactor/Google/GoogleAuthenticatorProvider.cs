...
namespace prod.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : prodServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("prod", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);

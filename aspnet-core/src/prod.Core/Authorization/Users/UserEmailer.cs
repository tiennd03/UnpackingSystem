...
namespace prod.Authorization.Users
{
    ...
    /// </summary>
    public class UserEmailer : prodServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        public async Task SendPasswordResetLinkAsync(User user, string link = null)
        {
            ...
            if (!link.IsNullOrEmpty())
            {
                ...
                var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours).ToString(prodConsts.DateTimeOffsetFormat));
        ...
        private async Task SendAsync(string emailAddress, string subject, StringBuilder mailMessage)
        {
            await _emailSender.SendAsync(new MailMessage
            {
                To = { emailAddress },
                // To = { "helpdesk@toyotavn.com.vn" },
                CC = { "helpdesk@toyotavn.com.vn" },
                Subject = subject,
        ...
        public async Task SendEmailReportRequetsAsync(string userName, string dataListInput, string from, string to, string cc, string subject, bool isBodyHtml = true)
        {
            string[] data = dataListInput.Split(';');
            var userMailMessage = new StringBuilder();
            userMailMessage.AppendLine("<div >\r\n" +
                                       "<span> Dear " + userName + ",</span><br><br>\r\n        " +
                                       "<span> &emsp; Report Request của bạn đã được gửi lên hệ thống</span><br>\r\n" +
                                       "<span> &emsp; Hệ thống đã nhận và sẽ xử lý </span><br>\r\n <hr>\r\n        " +
                                       "<span> &emsp; REPORT REQUEST SUMMARY :</span><br><br>\r\n");
            if (data[0] == "true")
            {
                userMailMessage.AppendLine("<span> &emsp; &emsp;+ Import by Cont</span> <br>\r\n");
            }
            if (data[1] == "true")
            {
                userMailMessage.AppendLine("<span> &emsp; &emsp;+ Inv Stock Report</span> <br>\r\n");
            }
            if (data[2] == "true")
            {
                userMailMessage.AppendLine("<span> &emsp; &emsp;+ Inv Out Wip Stock Report</span> <br>\r\n");
            }
            if (data[3] == "true")
            {
                userMailMessage.AppendLine("<span> &emsp; &emsp;+ Inv In Out Line Off Report</span> <br>\r\n");
            }
            if (data[4] == "true")
            {
                userMailMessage.AppendLine(" <span> &emsp; &emsp;+ Inv In Details Report</span> <br><br>\r\n\r\n");
            }

            userMailMessage.AppendLine("<span>&emsp; &emsp;+ Daily/Montly Report Period : " + data[5] + "</span><br>\r\n");
            userMailMessage.AppendLine("<span>&emsp; &emsp;+ D125 Period : " + data[6] + "</span><br>\r\n");
            userMailMessage.AppendLine("<span>&emsp; &emsp;+ Grade List : " + data[7] + "</span><br>\r\n");
            userMailMessage.AppendLine("<span>&emsp; &emsp;+ Frame Name : " + data[8] + "</span><br>\r\n\r\n<hr>\r\n");

            userMailMessage.AppendLine("<span>&emsp;Bạn cần trợ giúp? Liên hệ với chúng tôi</span><br>\r\n       " +
                                        " <span>&emsp;Email: support@toyotavn.com.vn</span><br>\r\n       " +
                                        " <span>&emsp;Sdt: (861) 2222 </span><br><br>\r\n\r\n       " +
                                        "<span>&emsp; © 2022 Toyota Motor Vietnam</span></div>\r\n\r\n    </div>");

            await SendAsync(to, subject, userMailMessage);
        }

        #endregion Report Request 
    }
}
...
export class TenantSettingsComponent extends AppComponentBase implements OnInit {
    ...
    @ViewChild('uploadDarkLogoInputLabel') uploadDarkLogoInputLabel: ElementRef;
    @ViewChild('uploadLightLogoInputLabel') uploadLightLogoInputLabel: ElementRef;
    ...
    onUploadDarkLogoInputChange(files: FileList) {
        this.uploadDarkLogoInputLabel.nativeElement.innerText = Array.from(files)
            .map((f) => f.name)
            .join(', ');
    }

    onUploadLightLogoInputChange(files: FileList) {
        this.uploadLightLogoInputLabel.nativeElement.innerText = Array.from(files)
            .map((f) => f.name)
            .join(', ');
    }
